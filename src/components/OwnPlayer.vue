...
const props = defineProps({
  ...
  dataSource: {
    type: Array as PropType<{ author: string; title: string; url: string; pic: string; lrc?: number }[]>,
    required: true,
    default: [] as { author: string; title: string; url: string; pic: string; lrc?: number }[]
  }
})

// 初始化
onMounted(() => {
  const audioList = props.dataSource.map(value => new Audio(value.author, value.title, value.url, value.pic, 0));
